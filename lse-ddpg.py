    ...
    for global_step in range(args.total_timesteps):
        ...
        if global_step > args.learning_starts:
            ...
            if global_step % args.policy_frequency == 0:
                ...
                with torch.no_grad():
                    noise = np.random.multivariate_normal(np.zeros_like(actor.action_scale.cpu().numpy()).reshape(-1), np.diag(actor.action_scale.reshape(-1).cpu().numpy() * args.rs_noise), size=(args.lse_samples, args.batch_size))
